...
class UNetTrainer(object):
    def __init__(self, args):
        ...
        # Define Network
        model = UNet(args, num_classes=self.n_class)
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            ...
            image, target = image.to(device=self.args.cuda, dtype=torch.float), target.to(device=self.args.cuda)
    ...
    def validation(self, epoch):
        ...
        with tqdm(total=len(self.val_loader)) as tbar:
            for i, sample in enumerate(self.val_loader):
                image = sample['image'].to(device=self.args.cuda, dtype=torch.float)
...
def main():
    ...
    if args.checkname is None:
        args.checkname = 'unet-' + str('normal')
