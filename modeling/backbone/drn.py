...
class DRN(nn.Module):
    ...
    def __init__(self, args, block, layers, arch='D',
        ...
        if arch == 'C':
            self.conv1 = nn.Conv2d(self.args.inpust_channel, channels[0], kernel_size=7, stride=1,
            ...
            self.layer0 = nn.Sequential(
                nn.Conv2d(self.args.input_channel, channels[0], kernel_size=7, stride=1, padding=3,
                          bias=False),
...
def drn_d_54(args, BatchNorm, pretrained=True):
    model = DRN(args, Bottleneck, [1, 1, 3, 4, 6, 3, 1, 1], arch='D', BatchNorm=BatchNorm)
    if pretrained:
